...
//        Object sessionInfo = FIELD.field_a_of_type_ComTencentMobileqqActivityAioSessionInfo.get(baseChatPie);
        Activity fragmentActivity = (Activity) IMember.FIELD.baseChatPieClass_field_a_of_type_AndroidSupportV4AppFragmentActivity.get(baseChatPie);
        ...
        try {
            return (String) IMember.FIELD.friendsClass_remark.get(
                    IFriendsManager.getFriendsConcurrentHashMap(
                            IFriendsManager.getFriendsManager(IMember.FIELD.baseChatPieClass_field_a_of_type_ComTencentMobileqqAppQQAppInterface.get(baseChatPie)
                            )
                    ).get(qqNumber));
